...
First, the ~~elephant~~ _increasingly svelte puma_ in the room, GNOME Shell: Shell uses its own toolkit, st, rather than GTK. This work that's going into Libadwaita will thus need to be implemented in st as well. This will likely require implementing either CSS variables or mix and alpha functions that work at runtime—not just compile-time. There are also places in the Shell styling that use images—like checks and switches—that will either need to be ported to CSS, or we'll need to provide colored SVGs in all the accent colors.
